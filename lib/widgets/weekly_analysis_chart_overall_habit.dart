...
class WeeklyAnalysisChartOverallHabit extends StatefulWidget {
  final List<Habit> habit;
  const WeeklyAnalysisChartOverallHabit({super.key, required this.habit});

  @override
  _WeeklyAnalysisChartOverallHabitState createState() => _WeeklyAnalysisChartOverallHabitState();
}

class _WeeklyAnalysisChartOverallHabitState extends State<WeeklyAnalysisChartOverallHabit> {
  ...
  double getAverageProgressForDate(DateTime date) {
    ...
    // Calculate the average progress if at least one habit has progress on the given date
    if (habitCount > 0) {
  ...
  BarChartGroupData _makeGroupData(int x, DateTime date) {
    return BarChartGroupData(
      ...
      barRods: [
        BarChartRodData(
          toY: getAverageProgressForDate(date)!,
